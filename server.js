...
async function ensureColumn(table, column, definition) {
  ...
  // SQLite/Turso does not allow non-constant defaults in ALTER TABLE ADD COLUMN.
  if (/DEFAULT\s+CURRENT_TIMESTAMP/i.test(definition)) {
    const safeDef = definition.replace(/\s+DEFAULT\s+CURRENT_TIMESTAMP/ig, '');
    await q(`ALTER TABLE ${table} ADD COLUMN ${column} ${safeDef}`);
    await q(`UPDATE ${table} SET ${column}=CURRENT_TIMESTAMP WHERE ${column} IS NULL`);
    return;
  }
...
async function verifyRuntimeConfig() {
  ...
  if (process.env.NODE_ENV === 'production' && (!process.env.SESSION_SECRET || process.env.SESSION_SECRET.length < 32)) missing.push('SESSION_SECRET (32+ characters)');
  if (missing.length) throw new Error(`Missing/invalid Render environment variables: ${missing.join(', ')}`);
}
...
async function init() {
  ...
  const defaults = {
    store_name: 'R TEX BD', tagline: "Women’s Three-Piece Collection", phone: '01629380347', whatsapp: '8801629380347', email: '', address: 'Bangladesh', support_hours: '10:00 AM – 10:00 PM', delivery_inside: '80', delivery_outside: '130',
...
app.get('/health', async (req,res)=>{
  try { await q('SELECT 1 AS ok'); res.set('Cache-Control','no-store').json({ok:true,service:'rtex-bd',database:'connected'}); }
  catch(e) { res.status(503).json({ok:false,service:'rtex-bd',database:'unavailable'}); }
});
...
app.post('/api/admin/login',adminLimiter,async(req,res)=>{
  ...
  if(!hash)return res.status(500).json({error:'Admin login is not configured. Set ADMIN_USERNAME and ADMIN_PASSWORD (or ADMIN_PASSWORD_HASH) in Render Environment Variables, then redeploy.'});
...
app.put('/api/admin/settings',requireAdmin,async(req,res,next)=>{try{const allowed=['store_name','tagline','phone','whatsapp','email','address','support_hours','delivery_inside','delivery_outside','hero_title','hero_subtitle','about','footer_note'];for(const key of allowed){if(req.body[key]!==undefined)await q('INSERT INTO settings(key,value) VALUES(?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value',[key,String(req.body[key]).slice(0,5000)]);}res.json({ok:true});}catch(e){next(e)}});
...
async function buildChatContext() {
  const products = await allProducts({activeOnly:true,limit:100});
  const settings = await getSettings();
  return {settings, products};
}

// Human-style, database-grounded shopping assistant. This is intentionally coded
// as a real conversational decision engine instead of returning one generic reply.
// It understands Bangla, Banglish and English, remembers the recent conversation,
// extracts budgets/sizes/colors, searches the live catalog, and can check an order.
app.post('/api/chat',async(req,res,next)=>{try{
  ...
  const context=await buildChatContext(),history=Array.isArray(req.body.history)?req.body.history.filter(x=>x&&typeof x.content==='string').slice(-14):[];
  const orderMatch=message.match(/\bRTX-[A-Z0-9-]{5,40}\b/i);
  if(orderMatch){const order=await one('SELECT order_number,status,payment_status,created_at,total FROM orders WHERE order_number=?',[orderMatch[0].toUpperCase()]);if(order)return res.json({reply:`আপনার ${order.order_number} order-এর current status **${order.status}** এবং payment status **${order.payment_status}**। Total ${moneyBDT(order.total)}।`,products:[]});return res.json({reply:`আমি ${orderMatch[0].toUpperCase()} order number-টা খুঁজে পাইনি। Numberটা আরেকবার মিলিয়ে পাঠান।`,products:[]});}
  res.json(answerFromCatalog(context,message,history));
}catch(e){next(e)}});
...
app.use((err,req,res,next)=>{
  ...
  if(wantsHtml) return res.status(500).send('<!doctype html><html><head><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>R TEX BD</title></head><body style=\"font-family:Arial,sans-serif;padding:40px;text-align:center\"><h1>Something went wrong</h1><p>Please try again in a moment.</p></body></html>');
